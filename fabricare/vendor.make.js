...
var compileProject = {
  ...
  "cSource" : [
    "source/blocksort.c", "source/huffman.c", "source/crctable.c",
    "source/randtable.c", "source/compress.c", "source/decompress.c",
    "source/bzlib.c"
  ],
  "linkerDefinitionsFile" : "source/libbz2.def"
};

Shell.filePutContents("temp/" + compileProject.project + ".compile.json",
                      JSON.encodeWithIndentation(compileProject));
exitIf(xyoCC.apply(
    null, xyoCCExtra("@temp/" + compileProject.project + ".compile.json",
                     "--lib", "--output-lib-path=output/lib", "--crt-static")));
exitIf(xyoCC.apply(
    null, xyoCCExtra("@temp/" + compileProject.project + ".compile.json",
                     "--dll", "--output-bin-path=output/bin",
                     "--output-lib-path=output/lib")));
...
exitIf(xyoCC.apply(
    null, xyoCCExtra("@temp/" + compileProject.project + ".compile.json",
                     "--exe", "--output-bin-path=output/bin", "--crt-static")));
